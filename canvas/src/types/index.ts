...
export class LWWMap<T> {
  ...
  constructor(id: string, state: State<T>) {
    ...
    for (const [key, register] of (Object as any).entries(state)) {
  ...
  merge(state: State<T>) {
    for (const [key, remote] of (Object as any).entries(state)) {
      const local = this.#data.get(key);

      if (local) local.merge(remote); // 使用上面的register的合并操作
